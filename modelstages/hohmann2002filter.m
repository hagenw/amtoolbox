function filter = hohmann2002filter(arg1,arg2,arg3,arg4,arg5)
%HOHMANN2002FILTER   Construct a Gammatone filter within the HOHMANN2002 framework
%   Usage:  hohmann2002filter(a_tilde,gamma_order);
%           hohmann2002filter(fs, fc, gamma_order);
%           hohmann2002filter(fs, fc, gamma_order, bandwidth_factor);
%           hohmann2002filter(fs, fc, bw, attenuation_db, gamma_order);
%
%   Input arguments:
...
% 
%   `hohmann2002filter(a_tilde, gamma_order)` specifies the complex-valued
%   filter coefficients directly as at the beginning of Sec. 2.3 in Hohmann
%   (2002). 
%
%   `hohmann2002filter(fs, fc, gamma_order)` computes
...
%
%   `hohmann2002filter(fs, fc, gamma_order, bandwidth_factor)` uses filter 
%   bandwidth of `bandwidth_factor` ERB (instead of 1 ERB).
%
%   `hohmann2002filter(fs, fc, bw, attenuation_db, gamma_order)` designs
