...
for ll = 1:length(s)
  ...
  for ii = 1:length(kv.latseg)
    ...
    [s(ll).p{ii},respangs,polang] = baumgartner2016(...
        s(ll).Obj,s(ll).Obj,{s(ll).id},'fs',s(ll).fs,...
        'lat',kv.latseg(ii),...
        'mrsmsp',s(ll).mrs,'S',S,'do',kv.do,...
        'stim',stim,'fsstim',s(ll).fsstim,...
        'prior',kv.prior,'priordist',s(ll).priordist,...
        'SPL',SPL,'SPLtem',kv.SPLtem,'gamma',kv.gamma,...
        'tiwin',kv.tiwin,'mgs',kv.mgs,...
