function [out, signal1, signal2]=stimulus(Fs,mode, len,f,bw,itd1,itd2,isi,at,dc,nn,lag_level)
...
ISI=abs(isi);
signal1=signal(Fs,mode,len,f,bw,itd1,1,at,dc); % ILD is the difference in level between the ipsilteral and contralateral ears
signal2=signal(Fs,mode,len,f,bw,itd2,1,at,dc); % ILD should be a negative number so that the contralateral ear is attenuated
