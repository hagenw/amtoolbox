...
%
%     [dataOut] = exp_kelvasa2015('fig8a','identifier','BTE',...
%                                         'HRTFchannels',[3,4]);
%          Reproduces data for paper figure 8a using the behind-the-ear
%          microphone channel('BTE')
%          
%     [dataOut] = exp_kelvasa2015('fig8a',varargin);
%          Reproduces model figure data for fig8a with updated model
...
if flags.do_fig5
    ...
    for ind = 1 : 8
        ...
        [signal, fs] = audioread(signals{ceil(ind./2)}); 
...
if flags.do_fig7
        ...
        [signal, fs] = audioread(signalName{sig});
        signal = signal(1:6*fs,:);
        signal = resample(signal,kv.FS_ACE,fs);
...
if flags.do_fig12
       ...
       kv.AziCrvfitRange =  90;
        ...
        azis = -180:5:175;
        [noise fs] = audioread('PinkNoise.wav');
