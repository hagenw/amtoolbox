...
%               Error bars: 짹1 standard deviation of the average. 
...
%               loudspeaker in the same SP (lateral angle: 30째). 
...
%               and NH12. 
%               Left: Span of 0째 (single-loudspeaker reproduction, baseline). 
%               Center: Span of 30째. 
%               Right: Span of 60째. 
...
%               Center: 10.2 setup (DSX), panning from L (polar angle of 0째) 
%               via LH2 (55째) to LS (180째). 
%               Right: 9.1 setup (Auro-3D), panning from L (0째) via 
%               LH1 (34째) and LSH (121째) to LS (180째). 
...
%               at the azimuth of 110째. 
%               Center: LSH at the azimuth of 130째. 
%               Right: LS and LSH at the azimuth of 130째. 
...
if flags.do_fig13 || flags.do_fig14 || flags.do_fig15
  ...
  if flags.do_fig13 % -----------------------------------------------------
    ...
    if flags.do_plot
      ...
      ylabel('Local Polar RMS Error (째)')
      xlabel({'NH'})
  ...
  elseif flags.do_fig14 % -------------------------------------------------
    ...
    if flags.do_plot
      ...
      ylabel('Increase in Local Polar RMS Error (째)')
      xlabel('NH')
      set(h,'FaceColor','white')
