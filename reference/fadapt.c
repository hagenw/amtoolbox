/* 2010: Comment by P. Soendergaard:
 *
 * This is the original adaptation loop implemenentation used by Jeoen
 * Breebarty used in his thesis. This implementation is slightly
 * different from the standard adaptation loop implementation. Where
 * as the standard implementation reaches a steady state for steady
 * state input, this implemenation produces slightly growing output.
 *
 * This implementation can be written in Matlab as:
 *
 ...
 *      for jj=1:nloops
 *
 *        state(jj) = a1(jj)*state(jj) + b0(jj)*tmp1; % Reverse order !      
 *        tmp1=tmp1/state(jj);                        % Reverse order !
 *      
 *      end;
 *      
 *      % store the result.
 ...
 *
 *
 *  The two lines in the middle are in the opposite order as compared
 *  to the standard adaptation loop implementation.
 *
 *  Thanks to Anine Poulsen for providing this information.
 */
